...
function InformationPage({ members, collections, onAdd, onEdit, onDelete }) {
  ...
  return <div className="dashboard-content"><div className="page-heading"><div><span className="eyebrow">MEMBER DIRECTORY</span><h1>Member&apos;s Profile</h1><p>Manage members and view every contribution type.</p></div><button className="button button-primary" onClick={onAdd}><Plus size={17} /> Add member profile</button></div><section className="panel members-panel"><div className="table-toolbar"><div className="search-field"><Search size={17} /><input placeholder="Search members" /></div><button className="select-button">All positions <ChevronDown size={15} /></button></div><div className="member-grid">{memberRows.map((item) => <article className="member-card" key={item.id}><div className="member-card-header"><div className="member-cell"><Avatar initials={item.photo} src={item.photo?.startsWith('data:') ? item.photo : undefined} color={item.color} /><span><strong>{item.name}</strong><small>{item.position}</small></span></div><div className="member-actions"><button className="row-more" onClick={() => setOpenMenu(openMenu === item.id ? null : item.id)} aria-label={`Actions for ${item.name}`}>•••</button>{openMenu === item.id && <div className="member-menu"><button onClick={() => { setOpenMenu(null); onEdit(item); }}>Edit</button><button className="danger-action" onClick={() => { setOpenMenu(null); onDelete(item); }}>Delete</button></div>}</div></div><div className="contribution-grid"><div><span>Event collection</span><strong>{item.totals['Event Collection'] ? formatCurrency(item.totals['Event Collection']) : '—'}</strong></div><div><span>Locale collection</span><strong>{item.totals['Locale Collection'] ? formatCurrency(item.totals['Locale Collection']) : '—'}</strong></div><div><span>Weekly dues</span><strong>{item.totals['Weekly Dues Collection'] ? formatCurrency(item.totals['Weekly Dues Collection']) : '—'}</strong></div></div></article>)}</div></section></div>;
}
