...
function ImageCropper({ source, onCancel, onSave }) {
  ...
  const moveCrop = (event) => {
    if (!dragStart) return;
    setOffsetX(Math.max(-100, Math.min(100, dragStart.x + (event.clientX - dragStart.clientX) / 2)));
    setOffsetY(Math.max(-100, Math.min(100, dragStart.y + (event.clientY - dragStart.clientY) / 2)));
  };
  const saveCrop = () => {
  ...
  return <div className="modal-backdrop crop-backdrop"><div className="modal crop-modal"><div className="modal-heading"><div><span className="eyebrow">PROFILE PHOTO</span><h2>Crop your picture</h2><p>Drag the image freely or adjust the zoom.</p></div><button className="close-button" onClick={onCancel}><X size={19} /></button></div><div className="crop-preview" onPointerDown={(event) => { event.currentTarget.setPointerCapture(event.pointerId); setDragStart({ x: offsetX, y: offsetY, clientX: event.clientX, clientY: event.clientY }); }} onPointerMove={moveCrop} onPointerUp={() => setDragStart(null)} onPointerCancel={() => setDragStart(null)}><img src={source} alt="Crop preview" style={{ transform: `translate(${offsetX / 4}%, ${offsetY / 4}%) scale(${zoom})` }} /></div><div className="crop-controls"><label>Zoom<input type="range" min="1" max="3" step=".05" value={zoom} onChange={(event) => setZoom(Number(event.target.value))} /></label></div><div className="modal-actions"><button type="button" className="button button-quiet" onClick={onCancel}>Cancel</button><button type="button" className="button button-primary" onClick={saveCrop}><Check size={16} /> Use this photo</button></div></div></div>;
}
...
const mentorsWeeklyRate = { Student: 30, 'Working Student': 40, Employed: 50, Unemployed: 10 };
...
function Avatar({ initials, color = 'blue', src }) {
  const source = imageSource(src) || imageSource(initials);
  return source ? <img className="avatar avatar-image" src={source} alt="" onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = logoImage; }} /> : <div className={`avatar avatar-${color}`}>{initials}</div>;
}
...
function Sidebar({ active, setActive, onLogout, group, onSettings, onSwitchGroup, onBackAdmin, isAdministrator, isAdminGroupView, profileImage }) {
  const items = [
...
function CollectionRow({ collection }) {
  return <div className="collection-row"><Avatar initials={collection.photo} src={imageSource(collection.photo)} color={collection.color} /><div className="collection-person"><strong>{collection.name}</strong><span>{collection.position}</span></div><span className={`type-pill ${typeClass(collection.type)}`}>{collection.type}</span><strong className="row-amount">{formatCurrency(collection.amount)}</strong><span className="row-date">{new Date(`${collection.date}T00:00:00`).toLocaleDateString('en', { month: 'short', day: 'numeric', year: 'numeric' })}</span><button className="row-more">•••</button></div>;
}
...
function MentorsDashboard({ members, collections, onAdd, onApplyWeeklyDues }) {
  ...
  const totalPaid = memberRows.reduce((sum, member) => sum + member.weeklyPaid, 0);
  ...
  return <div className="dashboard-content"><div className="page-heading"><div><span className="eyebrow">MENTORS DASHBOARD · {monthLabel.toUpperCase()}</span><h1>Overview</h1><p>Track cashflow, weekly rates, and unpaid balances for every member.</p></div><button className="button button-primary" onClick={onAdd}><Plus size={17} /> Add collection</button></div><div className="stats-grid mentors-summary-grid"><StatCard title="Weekly cashflow" amount={totalPaid} icon={CircleDollarSign} tone="navy" change="Paid dues" /><StatCard title="Unpaid balance" amount={totalUnpaid} icon={WalletCards} tone="orange" change="Outstanding" /><StatCard title="Weekly expected" amount={weeklyExpected} icon={CalendarDays} tone="yellow" change="By status" />  <StatCard title="Members" amount={members.length} icon={UsersRound} tone="blue" change="Registered" /></div><section className="panel mentors-dashboard-panel"><div className="panel-heading"><div><span className="eyebrow">EVENT COLLECTIONS</span><h2>Event collection</h2><p>{formatCurrency(eventCollections.reduce((sum, item) => sum + item.amount, 0))} total</p></div><button className="button button-quiet" onClick={onAdd}>Add event</button></div><div className="collection-list">{eventCollections.slice(0, 3).map((item) => <CollectionRow key={item.id} collection={item} />)}</div></section><section className="panel mentors-dashboard-panel"><div className="panel-heading"><div><span className="eyebrow">MEMBER CASHFLOW</span><h2>Members and unpaid balance</h2><p>Weekly payment is based on each member&apos;s status.</p></div><button className="button button-quiet" onClick={onApplyWeeklyDues}>Apply this week&apos;s dues</button></div><div className="mentors-cashflow-list">{memberRows.length ? memberRows.map((member) => <article className="mentors-cashflow-row" key={member.id}><Avatar initials={member.photo} src={imageSource(member.photo)} color={member.color} /><div className="collection-person"><strong>{member.name}</strong><span>{member.position} · {member.status}</span></div><div><small>Weekly rate</small><strong>{formatCurrency(member.weeklyRate)}</strong></div><div><small>Paid dues</small><strong className="cashflow-paid">{formatCurrency(member.weeklyPaid)}</strong></div><div><small>Unpaid balance</small><strong className={member.unpaidBalance ? 'cashflow-unpaid' : 'cashflow-settled'}>{formatCurrency(member.unpaidBalance)}</strong></div></article>) : <div className="empty-ledger">No Mentors members have been registered yet.</div>}</div></section></div>;
}
...
function AddCollection({ members, onSubmit, onCancel }) {
  ...
  const [form, setForm] = useState({ memberId: firstMember?.id || '', amount: '', type: 'Event Collection', eventName: '', date: today });
  ...
  const submit = (e) => {
    e.preventDefault();
    if ((form.type !== 'Event Collection' && !selectedMember) || !form.amount) return;
    const isEvent = form.type === 'Event Collection';
    onSubmit({
      ...form,
      id: Date.now(),
      memberId: isEvent ? null : selectedMember.id,
      name: isEvent ? (form.eventName.trim() || 'Unnamed event') : selectedMember.name,
      position: isEvent ? 'Event' : selectedMember.position,
      amount: Number(form.amount),
      photo: isEvent ? (form.eventName.trim() || 'EV').split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase() : selectedMember.photo,
      color: isEvent ? 'gold' : selectedMember.color,
    });
  };
  return <div className="modal-backdrop"><div className="modal"><div className="modal-heading"><div><span className="eyebrow">NEW ENTRY</span><h2>Add collection</h2><p>{form.type === 'Event Collection' ? 'Record an event-level collection.' : 'Select a member and record their contribution.'}</p></div><button className="close-button" onClick={onCancel}><X size={19} /></button></div><form onSubmit={submit} className="modal-form">{form.type === 'Event Collection' ? <label>Event name<input autoFocus value={form.eventName} onChange={(e) => update('eventName', e.target.value)} placeholder="e.g. Anniversary celebration" /></label> : <label>Full name<select autoFocus value={form.memberId} onChange={(e) => selectMember(e.target.value)} disabled={!members.length}><option value="" disabled>{members.length ? 'Select a member' : 'Add member profile first'}</option>{members.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></label>}<div className="form-row"><label>Position<input value={form.type === 'Event Collection' ? 'Event' : selectedMember?.position || ''} readOnly placeholder="Selected member position" /></label><label>Amount<input type="number" min="0" value={form.amount} onChange={(e) => update('amount', e.target.value)} placeholder="0.00" /></label></div><div className="form-row"><label>Collection type<select value={form.type} onChange={(e) => update('type', e.target.value)}><option>Event Collection</option><option>Locale Collection</option><option>Weekly Dues Collection</option></select></label><label>Date<input type="date" value={form.date} onChange={(e) => update('date', e.target.value)} /></label></div><div className="modal-actions"><button type="button" className="button button-quiet" onClick={onCancel}>Cancel</button><button className="button button-primary" type="submit" disabled={form.type !== 'Event Collection' && !selectedMember}><Check size={16} /> Save collection</button></div></form></div></div>;
}
...
function AddWeeklyBalance({ onSubmit, onCancel }) {
  const [amount, setAmount] = useState('');
  return <div className="modal-backdrop"><div className="modal"><div className="modal-heading"><div><span className="eyebrow">MENTORS WEEKLY DUES</span><h2>Add weekly unpaid balance</h2><p>Add this amount to every Mentors member&apos;s unpaid balance.</p></div><button className="close-button" onClick={onCancel}><X size={19} /></button></div><form className="modal-form" onSubmit={(event) => { event.preventDefault(); if (Number(amount) > 0) onSubmit(Number(amount)); }}><label>Weekly amount<input autoFocus type="number" min="1" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="0.00" /></label><div className="modal-actions"><button type="button" className="button button-quiet" onClick={onCancel}>Cancel</button><button className="button button-primary" type="submit"><Check size={16} /> Add balance</button></div></form></div></div>;
}

function InformationPage({ members, collections, onAdd, onEdit, onDelete, onAddWeeklyDue, readOnly = false, isMentors = false }) {
...
function AddMember({ member, onSubmit, onCancel, isMentors = false }) {
  ...
  const submit = (event) => {
    ...
    onSubmit({
      ...
      status: isMentors ? form.status : undefined,
      gender: form.gender,
      unpaidBalance: isMentors ? Number(form.unpaidBalance) || 0 : undefined,
    });
  };
...
function App() {
  ...
  }, [report, receipts, sheetLink, group]);
  useEffect(() => {
  ...
  const addCollection = (item) => { setCollections((current) => [item, ...current]); if (group === 'Mentors' && item.type === 'Weekly Dues Collection') setMembers((current) => current.map((member) => member.id === item.memberId ? { ...member, unpaidBalance: Math.max(0, (Number(member.unpaidBalance) || 0) - item.amount) } : member)); setModal(false); setActive('Dashboard'); };
