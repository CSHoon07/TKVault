...
function MentorsDashboard({ members, collections, onAdd, onAddEvent = () => { localStorage.setItem('tkvault-event-mode', 'true'); onAdd(); }, onApplyWeeklyDues }) {
...
function AddCollection({ members, onSubmit, onCancel, eventMode = localStorage.getItem('tkvault-event-mode') === 'true' }) {
  const firstMember = members[0];
  const [form, setForm] = useState({ memberId: firstMember?.id || '', amount: '', type: eventMode ? 'Event Collection' : 'Locale Collection', eventName: '', contributorName: '', position: 'Provincial', date: today });
  ...
  return <div className="modal-backdrop"><div className="modal"><div className="modal-heading"><div><span className="eyebrow">NEW ENTRY</span><h2>{eventMode ? 'Add event collection' : 'Add member collection'}</h2><p>{eventMode ? 'Record the contributor and event details.' : 'Select a registered member and record their contribution.'}</p></div><button className="close-button" onClick={onCancel}><X size={19} /></button></div><form onSubmit={submit} className="modal-form">{eventMode ? <><label>Event name<input autoFocus value={form.eventName} onChange={(e) => update('eventName', e.target.value)} placeholder="e.g. Anniversary celebration" /></label><label>Contributor name<input value={form.contributorName} onChange={(e) => update('contributorName', e.target.value)} placeholder="Enter contributor name" /></label><div className="form-row"><label>Position<select className="goal-select modal-select" value={form.position} onChange={(e) => update('position', e.target.value)}><option>Provincial</option><option>City</option><option>Locale</option><option>K&amp;T</option></select></label><label>Amount<input type="number" min="0" value={form.amount} onChange={(e) => update('amount', e.target.value)} placeholder="0.00" /></label></div></> : <><label>Member<select autoFocus value={form.memberId} onChange={(e) => selectMember(e.target.value)} disabled={!members.length}><option value="" disabled>{members.length ? 'Select a member' : 'Add member profile first'}</option>{members.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></label><div className="form-row"><label>Position<input value={selectedMember?.position || ''} readOnly placeholder="Selected member position" /></label><label>Amount<input type="number" min="0" value={form.amount} onChange={(e) => update('amount', e.target.value)} placeholder="0.00" /></label></div></>}<div className="form-row"><label>Collection type<select value={form.type} onChange={(e) => update('type', e.target.value)}>{eventMode ? <option>Event Collection</option> : <><option>Locale Collection</option><option>Weekly Dues Collection</option></>}</select></label><label>Date<input type="date" value={form.date} onChange={(e) => update('date', e.target.value)} /></label></div><div className="modal-actions"><button type="button" className="button button-quiet" onClick={onCancel}>Cancel</button><button className="button button-primary" type="submit" disabled={eventMode ? !form.contributorName.trim() : !selectedMember}><Check size={16} /> Save collection</button></div></form></div></div>;
}
