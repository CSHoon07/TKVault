...
function WeeklyDuesUsage({ usages = [], onChange }) {
  ...
  const edit = (usage) => {
    const nextName = window.prompt('Usage name', usage.name);
    const nextAmount = window.prompt('Amount used', String(usage.amount));
    if (nextName?.trim() && Number(nextAmount) > 0) onChange(usages.map((item) => item.id === usage.id ? { ...item, name: nextName.trim(), amount: Number(nextAmount) } : item));
  };
  const remove = (usage) => {
    if (window.confirm(`Delete ${usage.name}?`)) onChange(usages.filter((item) => item.id !== usage.id));
  };
