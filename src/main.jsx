...
function App() {
  ...
  const addWeeklyBalance = (amount) => { setMembers((current) => current.map((member) => ({ ...member, unpaidBalance: (Number(member.unpaidBalance) || 0) + amount }))); setModal(false); };
  ...
  return <div className={`app-shell group-theme-${groupTheme}`}><Sidebar group={isAdministrator ? 'Administrator' : group} profileImage={profileImage} isAdministrator={isAdministrator} isAdminGroupView={adminGroupView} onSwitchGroup={() => { setGroup(''); setAdminGroupView(false); setProfileReadOnly(false); setActive('Dashboard'); setMenuOpen(false); }} onBackAdmin={() => { setAdminGroupView(false); setGroup(groups[0].name); setProfileReadOnly(false); setActive('Dashboard'); setMenuOpen(false); }} active={active} setActive={(item) => { setProfileReadOnly(false); setActive(item); setMenuOpen(false); }} onSettings={() => { setActive('Settings'); setMenuOpen(false); }} onLogout={logout} /><div className={`mobile-overlay ${menuOpen ? 'show' : ''}`} onClick={() => setMenuOpen(false)} /><main className="main-area"><Header group={isAdministrator ? 'Administrator' : group} profileImage={profileImage} onProfile={() => { setProfileReadOnly(!isAdministrator); setActive(isAdministrator ? 'Admin Profile' : "Member's Profile"); }} onMenu={() => setMenuOpen(true)} />{active === 'Dashboard' && (isAdministrator ? <AdministratorDashboard collections={allGroupCollections} groups={groups} /> : <Dashboard collections={collections} goal={goal} onEditGoal={() => setModal('goal')} onViewAll={() => setActive('Collections')} onAdd={() => { setActive('Collections'); setModal('collection'); }} />)}{active === 'Admin Profile' && isAdministrator && <AdministratorProfile members={allGroupMembers} />  }{active === "Member's Profile" && !isAdministrator && <InformationPage members={members} collections={collections} isMentors={group === 'Mentors'} readOnly={profileReadOnly} onAdd={() => setModal('member')} onAddWeeklyDue={() => setModal('weekly-balance')} onEdit={(member) => setModal({ type: 'edit-member', member })} onDelete={deleteMember} />}{active === 'Collections' && <CollectionsPage collections={collections} onAdd={() => setModal('collection')} />}{active === 'Liquidation Report' && <LiquidationPage report={report} onReportChange={setReport} receipts={receipts} onReceiptChange={setReceipts} />}{active === 'Settings' && <SettingsPage account={account} group={group} profileImage={profileImage} onProfileChange={updateProfile} isAdministrator={account.role === 'administrator'} />}{active === 'Administrator Monitor' && isAdministrator && <AdministratorMonitor collections={allGroupCollections} />}</main>  {modal === 'member' && <AddMember isMentors={group === 'Mentors'} onSubmit={addMember} onCancel={() => setModal(false)} />}{modal?.type === 'edit-member' && <AddMember isMentors={group === 'Mentors'} member={modal.member} onSubmit={editMember} onCancel={() => setModal(false)} />} {modal === 'weekly-balance' && <AddWeeklyBalance onSubmit={addWeeklyBalance} onCancel={() => setModal(false)} />} {modal === 'collection' && <AddCollection members={members} onSubmit={addCollection} onCancel={() => setModal(false)} />}{modal === 'goal' && <EditGoal goal={goal} onSubmit={(value) => { setGoal(value); setModal(false); }} onCancel={() => setModal(false)} />}</div>;
}
